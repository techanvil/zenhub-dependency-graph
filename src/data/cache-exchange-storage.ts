/*
 * Copied from `@urql/exchange-graphcache` in order to modify the timestamp to use a granulaity of minutes rather than days:
 * https://github.com/urql-graphql/urql/blob/38290e71cb95d678fded2d367ef31792d2bf5ea5/exchanges/graphcache/src/default-storage/index.ts
 *
 * TODO: Address this in a more sustainable way.
 */
...
export interface StorageOptions {
   ...
   */
  idbName?: string;
  /** Maximum age of cache entries (in minutes) after which data is discarded.
...
export const makeDefaultStorage = (opts?: StorageOptions): DefaultStorage => {
  ...
  const timestamp = Math.floor(new Date().valueOf() / (1000 * 60));
  const maxAge = timestamp - (opts.maxAge || 60);
